...
export const Testimonial = () => {
  ...
  return (
    <section className="w-full h-full px-[24px] py-7 flex flex-col justify-center items-center gap-10">
      ...
      /> */}
      <h3 className="w-full text-[32px] font-extrabold font-Gilroy md:text-[40px] lg:text-[42px] max-w-[1200px] text-center text-secondary leading-tight">
        Testimonial
      </h3>
      <div className="relative w-full h-full mb-10 slider-container md:w-3/5 lg:w-1/2 xl:w-2/5">
        <Slider {...settings}>
          {testimonialList.map((list, id) => (
            <div
              key={id}
              className="w-full mx-auto space-y-5 text-base font-light leading-snug text-center text-secondary md:text-lg font-WorkSans"
            >
              <p>{list.review}</p>
              <h4>- {list.name} -</h4>
            </div>
